...
export default function BoltStatsPage() {
  ...
  const handleExportAll = async () => {
    ...
    try {
      ...
      localSheets.forEach(({ id, name }) => {
        const rows = (map[id] ?? []).map((r: any) => {
          ...
          EXPORT_COLUMNS.forEach(({ key, label }) => {
            ...
            rec[label] = dbFieldMap[key as string] ?? '';
          });
      ...
      XLSX.writeFile(wb, `螺栓生产良率记录_全部_${new Date().toISOString().slice(0, 10)}.xlsx`);
      showToast('全部工作表导出成功');
    } catch (err: any) {
  ...
  const parseNoteForDefectTypes = (note: string, record: BoltProductionRecord): Record<string, number> => {
    ...
    const parseChineseNumber = (s: string): { qty: number; rest: string } | null => {
      ...
      const match1 = s.match(/^([零一二二三三四五六七八九十两]+)\s*(个|颗|粒)\s*(.+)$/);
      if (match1) {
      ...
      const match2 = s.match(/^([零一二二三三四五六七八九十两]+)\s*(.+)$/);
      if (match2) {
    ...
    const parseDescription = (description: string, totalCount: number, typeMap: Record<string, number>): void => {
      ...
      for (const item of items) {
        // 格式A："XXX×2" 或 "XXX*2"（支持无空格，如"箔边爆破×3"）
        const qtySuffixMatch = item.match(/^(.+?)[×xX*](\d+)$/);
        if (qtySuffixMatch) {
        ...
        // 格式D："XXX2个"、"XXX15颗"（数字在前，如"箔边爆破3个"）
        const qtyPrefixMatch = item.match(/^(.+?)(\d+)(\s*)(个|颗|粒)(.*)$/);
        if (qtyPrefixMatch) {
          const defectType = qtyPrefixMatch[1].trim();
          const qty = parseInt(qtyPrefixMatch[2], 10);
          const suffix = qtyPrefixMatch[5].replace(/的情况$/, '').trim();
          const finalType = suffix || defectType;
          if (finalType) {
            parsedItems.push({ defectType: finalType, qty });
            hasExplicitQty = true;
          }
          continue;
        }
        // 只有"个/颗"前缀但无数字：添加到parsedItems，qty=0
        if (/^(?:个|颗|粒)/.test(item)) {
          const rest = item.replace(/^(?:个|颗|粒)/, '').trim();
          if (rest) parsedItems.push({ defectType: rest, qty: 0 });
          continue;
        }
        // 只有不良类型名称（无数量）：添加到parsedItems，qty=0
      ...
      if (hasExplicitQty) {
        ...
        if (parsedItems.length > 0 && totalCount > 0) {
          const base = Math.floor(totalCount / parsedItems.length);
          const extra = totalCount % parsedItems.length;
          for (let i = 0; i < parsedItems.length; i++) {
            const { defectType } = parsedItems[i];
    ...
    for (const part of parts) {
      ...
      if (!colonMatch) {
        ...
        for (const item of items) {
          // 尝试解析中文数字（两颗箔边爆破 / 十五箔边爆破）
          const cnResult = parseChineseNumber(item);
          if (cnResult) {
            typeMap[cnResult.rest] = (typeMap[cnResult.rest] || 0) + cnResult.qty;
            continue;
          }
          // 尝试解析 "XXX×数量" 格式（支持无空格，如"箔边爆破×3"、"箔边爆破x3"）
          const qtyMatch = item.match(/^(.+?)[×xX*](\d+)$/);
          if (qtyMatch) {
          ...
          if (qtyPrefixMatch) {
            ...
            const suffix = qtyPrefixMatch[5].replace(/的情况$/, '').trim();
            if (suffix) {
              typeMap[suffix] = (typeMap[suffix] || 0) + qty;
            } else if (defectType) {
              typeMap[defectType] = (typeMap[defectType] || 0) + qty;
            }
            continue;
    ...
    <div className="max-w-full mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-b-2xl rounded-tr-2xl p-5 shadow-sm">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="min-h-[400px] max-h-[calc(100vh-280px)] overflow-auto rounded-xl border border-gray-200 shadow-sm">
...
<thead className="sticky top-0 z-10 bg-gray-50">
                <tr>
                  ...
                  <tr className="bg-indigo-50 border-t-2 border-indigo-200 font-bold text-xs">
                    ...
                    <td className="px-2 py-1.5 border border-gray-200" style={{width:'130px'}}></td>
                    <td className="px-2 py-1.5 border border-gray-200" style={{width:'80px'}}></td>
                    <td className="px-2 py-1.5 border border-gray-200" style={{width:'110px'}}></td>
                    <td className="px-2 py-1.5 border border-gray-200" style={{width:'120px'}}></td>
                    <td className="px-2 py-1.5 text-center text-indigo-900 border border-gray-200" style={{width:'100px'}}>{summary.totalDesign.toLocaleString()}</td>
                    <td className="px-2 py-1.5 text-center text-indigo-600 font-medium border border-gray-200" style={{width:'120px'}}>{summary.totalActual.toLocaleString()}</td>
                    ...
                    <td className="px-2 py-1.5 text-center text-red-500 border border-gray-200" style={{width:'110px'}}>{formatNum(summary.firstBSBRate)}%</td>
                    ...
                    <td className="px-2 py-1.5 text-center text-indigo-600 border border-gray-200" style={{width:'60px'}}>{summary.defectVoltage > 0 ? formatNum(summary.defectVoltage) : "—"}</td>
                    <td className="px-2 py-1.5 text-center text-indigo-600 border border-gray-200" style={{width:'60px'}}>{summary.defectAppearance > 0 ? formatNum(summary.defectAppearance) : "—"}</td>
                    <td className="px-2 py-1.5 text-center text-indigo-600 border border-gray-200" style={{width:'60px'}}>{summary.defectLeakage > 0 ? formatNum(summary.defectLeakage) : "—"}</td>
                    <td className="px-2 py-1.5 text-center text-indigo-600 border border-gray-200" style={{width:'60px'}}>{summary.defectHighCap > 0 ? formatNum(summary.defectHighCap) : "—"}</td>
                    <td className="px-2 py-1.5 text-center text-indigo-600 border border-gray-200" style={{width:'60px'}}>{summary.defectLowCap > 0 ? formatNum(summary.defectLowCap) : "—"}</td>
                    <td className="px-2 py-1.5 text-center text-indigo-600 border border-gray-200" style={{width:'60px'}}>{summary.defectDF > 0 ? formatNum(summary.defectDF) : "—"}</td>
                    <td className="px-2 py-1.5 text-center text-indigo-400 border border-gray-200" style={{width:'80px'}}>—</td>
                    <td className="px-2 py-1.5 text-center text-indigo-400 border border-gray-200" style={{width:'170px'}}>—</td>
                    <td className="px-2 py-1.5 text-center text-indigo-400 border border-gray-200" style={{width:'110px'}}>—</td>
                    <td className="px-2 py-1.5 text-center text-indigo-400 border border-gray-200" style={{width:'70px'}}>—</td>
                  </tr>
                </tfoot><tbody>
                {filtered.map((r, i) => (
                  <tr key={r.id} className={`group hover:bg-indigo-50/40 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}`}>
                    ...
                    <td className="px-2 py-1.5 max-w-[170px] truncate border border-gray-200" title={r.notes}>{r.notes}</td>
